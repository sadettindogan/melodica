...
turkce_songs = {
    ...
    "Yok Bana Bu Cihanda": mp3_url("Yok Bana Bu Cihanda.mp3"),
    "Habibi": mp3_url("Habibi.mp3"),
}
...
# ANA EKRAN
# ---------------------------------------------------
DETECTIVE_IMG = f"{BASE}/detective.png"
col_title, col_img = st.columns([4, 1])
with col_title:
    st.title("🎵 Melodi Dedektifi")
with col_img:
    st.image(DETECTIVE_IMG, width=120)
        ...
        if letter_guess and len(letter_guess) == 1:
            if letter_guess in st.session_state.revealed_letters or letter_guess in st.session_state.wrong_letters:
                ...
                if is_fully_revealed():
                    ...
                    st.session_state.result_msg = f"🎉 Kazandınız! Tüm harfleri buldun! Doğru cevap: **{song}**"
